...
def _to_scalar_for_best(value):
    if isinstance(value, (list, tuple)):
        return float(value[0]) if len(value) > 0 else float("-inf")
...
def _add_readable_val_metrics(log_stats):
    rename_pairs = (
        ("test_AP_very_tiny", "val/AP_very_tiny"),
        ("test_AP_tiny", "val/AP_tiny"),
        ("test_AP_s", "val/AP_s"),
        ("test_AP_m", "val/AP_m"),
        ("test_AP_l", "val/AP_l"),
    )
    for src_key, dst_key in rename_pairs:
        if src_key in log_stats:
            log_stats[dst_key] = log_stats[src_key]
    ...
    def fit(self):
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if self.output_dir:
                ...
                # keep periodic checkpoints across all stages
            ...
            for k in test_stats:
                ...
                if self.writer and dist_utils.is_main_process():
                    if isinstance(test_stats[k], (list, tuple)):
                        for i, v in enumerate(test_stats[k]):
                            self.writer.add_scalar(f"Test/{k}_{i}".format(k), v, epoch)
                    else:
                        self.writer.add_scalar(f"Test/{k}", metric_value, epoch)

                if k in best_stat:
                    best_stat["epoch"] = (
                        epoch if metric_value > best_stat[k] else best_stat["epoch"]
                    )
                ...
                if best_stat["epoch"] == epoch and self.output_dir:
                    if epoch >= self.train_dataloader.collate_fn.stop_epoch:
                        if metric_value > top1:
                            top1 = metric_value
                            dist_utils.save_on_master(
                                self.state_dict(), self.output_dir / "best_stg2.pth"
                            )
                    else:
                        top1 = max(metric_value, top1)
                        dist_utils.save_on_master(
                            self.state_dict(), self.output_dir / "best_stg1.pth"
                        )
